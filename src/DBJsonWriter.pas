...
constructor TDBJsonWriter.Create(aObject: TJSONObject);
begin
  inherited Create;
  FObject := aObject;
end;

constructor TDBJsonWriter.Create;
begin
  inherited;
...
destructor TDBJsonWriter.Destroy;
begin
  if FOjectOwned then
    FObject.Free;
  inherited Destroy;
...
procedure TDBJsonWriter.WriteFields(aFields: TFields);
var
...
function TDBJsonWriter.NewArray(aFields: TArrayField) : TJSONArray;
const
...
procedure TDBJsonWriter.ApplyRecUpdate(aDataSet: TDataSet; aUpdateKind: TUpdateKind);
