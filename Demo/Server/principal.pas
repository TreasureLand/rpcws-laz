...
uses
  ...
  ExtCtrls, Menus, StdCtrls, ActnList, wsmethodsViews, httpdefs, fpHTTP, fpWeb,
  fpwebfile, fphttpapp, jwsconsts, jwsmessages, jwstypes, WSConnector,
  servermethods, fpjson, jsonparser, jwsmethods, fphttpserver, custweb,
  fpmimetypes, websession, Helper.Exceptions,
  //methods
  wsmethodsFuncoes,
  //acbr
  ACBrIBPTax
...
type
  ...
  private
    ...
    procedure DoJWSRequest(const ARequest: TJWSRequestContent; var AResponse: TJWSResponseContent);
    procedure ActionMethodsRequest(Sender: TObject; ARequest: TRequest;
                                   AResponse: TResponse; var Handled: Boolean);
  private
    { private declarations }
    FADDLogFile : Boolean;
    class var FModules: TJWSServerMethodsModules;
...
begin
  ...
  try
    try
      ...
      begin
        o.Free;
        raise Exception.Create('Não era um json object');
      end;
      ARequest := TJWSRequestContent.createAs(TJSONObject(o));
    except on
      e:exception do
      begin
        ARequest := nil;
...
procedure TFPWebM.DoJWSRequest(const ARequest: TJWSRequestContent; var AResponse: TJWSResponseContent);
var
  ReqMethods: TJWSServerMethods;
  cm: TJWSServerMethodsClass;
begin
  cm := FModules.GetClassMethod(ARequest);
  if cm = nil then
     raise Exception.Create('Modulo não encontrado');

  ReqMethods := cm.Create(ARequest);
  try
    //ReqMethods := TJWSMethods.Create;
    ReqMethods.ProcessRequest(AResponse);
  finally
  ...
  begin
    try
      try
        ...
        DoJWSRequest(jwsRequest,jwsResponse);//
...
class procedure TFPWebM.ClassInitialization;
begin
  try
    if FModules <> nil then
      raise Exception.Create('TFPWebM.ClassInitialization');
    FModules := TJWSServerMethodsModules.Create;
    FModules.RegisterClassModule('', TJWSMethods);
    FModules.RegisterClassModule('funcoes', TJWSMethodsFuncoes);
    FModules.RegisterClassModule('pessoas', TJWSMethodsPessoas);
    FModules.RegisterClassModule('views', TJWSMethodsViews);
    //FModules.RegisterClassModule('views', TJWSMethodsPessoas);
  except
    //Application.HandleException(Application);
    MessageDlg(Exception.GetMessageFrom(), mtError, [mbOK], 0);
    Abort;
  end;
end;
